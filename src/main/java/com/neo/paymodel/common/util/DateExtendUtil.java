...
public class DateExtendUtil {
	...
	public static String formatDate2FullDateString(String dateStr)
			throws Exception {
	...
	public static String formatDate2SmallDateString(String dateStr)
			throws Exception {
		Date date = parseString2Date(dateStr);
		return formatDate2String(date, SMALL_DATE_FORMAT);
	}
	...
	public static String formatDate2String(String dateStr, String formatString)
			throws Exception {
		Date date = parseString2Date(dateStr);
		return new SimpleDateFormat(formatString).format(date);
	}
	...
	public static String dateAdd(String datePart, int count, String date,
			String dateFormat) throws Exception // 日期加法操作
	{
		Date resultDate = null;
		Calendar cal = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Timestamp dateAddTimestamp(String datePart, int count,
			Timestamp date) // 日期加法操作
	{
	...
	public static Timestamp formartDate2Timestamp(Date targetDate) {
		Calendar cal = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Timestamp dateAddTimestampFromNow(String datePart, int count,
			Timestamp date) // 日期加法操作
	{
	...
	public static int getMonthDiff(String beginStr, String endStr)
		...
		int m = (beginCalendar.get(Calendar.MONTH))
	...
	public static long dateDiff(String datePart, Date date1, Date date2) {
		...
		// 月
		if ("month".equalsIgnoreCase(datePart)
				|| "m".equalsIgnoreCase(datePart)) {
			ret = (date2.getTime() - date1.getTime())
					/ (30 * 24 * 60 * 60 * 1000);
			return ret;
		}
		...
		// 分
		if ("minute".equalsIgnoreCase(datePart)
				|| "min".equalsIgnoreCase(datePart)) {
			ret = (date2.getTime() - date1.getTime()) / (1 * 60 * 1000);
			return ret;
		}
		// 秒
		if ("second".equalsIgnoreCase(datePart)
				|| "s".equalsIgnoreCase(datePart)) {
	 ...
	 * @throws Exception
	 */
	public static long dateDiff(String datePart, String date1, String date2)
			throws Exception {

		return dateDiff(datePart, parseString2Date(date1),
				parseString2Date(date2));
	}
	 ...
	 * @throws Exception
	 */
	public static long dateMonthDiff(String date1, String date2)
			throws Exception {

		return dateDiff("month", parseString2Date(date1),
				parseString2Date(date2));
	}
	 ...
	 * @throws Exception
	 */
	public static long dateYearDiff(String date1, String date2)
			throws Exception {

		return dateDiff("year", parseString2Date(date1),
				parseString2Date(date2));
	}
	 ...
	 * @return
	 */
	public static List<String> getAllBetweenTwoDate(String beginDate,
	...
	public static List<String> getWeekDateByWeekInYear(int year, int weekCount) {
		...
		for (int i = 0; i < 7; i++) {
			...
			String dayStr = (cal.get(Calendar.YEAR)) + "-"
	 ...
	 * @return
	 */
	public static List<String> getAllBetweenTwoWeek(String beginDate,
			String endDate) {
		List<String> weekLists = new ArrayList<String>();
		//
		List<String> days = getAllBetweenTwoDate(beginDate, endDate,
				SMALL_DATE_FORMAT);
		for (String day : days) {
	...
	/**
	 * 比较时间相差天数 -1起始时间大于结束时间 0同一天 -2 时间格式异常
	 */
	 ...
	 * @return
	 */
	public static List<String> getBaseListByMonth(String begin_time,
			String end_time) {
		List<String> returnList = new ArrayList<String>();
		//
		List<String> dayList = getAllBetweenTwoDate(begin_time, begin_time,
				DateExtendUtil.SMALL_DATE_FORMAT);
	 ...
	 * @return
	 */
	public static List<String> getBaseListByWeek(Calendar c_begin,
			Calendar c_end) {
		...
		String tempStr = "";
		//
		String timeStr = DateExtendUtil.formatDate2SmallDateString(c_begin
				.getTime());
		String endStr = DateExtendUtil.formatDate2SmallDateString(c_end
				.getTime());
		//
		...
		while (c_begin.before(c_end)) {
			if (temp != count) {
				...
				ret.add(timeStr);
				temp = count;
				timeStr = DateExtendUtil.formatDate2SmallDateString(c_begin
						.getTime());
			}
			if (c_begin.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY) {
				tempStr = DateExtendUtil.formatDate2SmallDateString(c_begin
						.getTime());
				count++;
			}
		...
		ret.add(timeStr + "~"
				+ endStr.substring(endStr.indexOf("-") + 1, endStr.length()));
	 ...
	 * @return
	 */
	public static List<String> getBaseListByDay(Date start, Date end,
			int calendarType) {
		...
		while (tmpDate.before(end) || tmpDate.getTime() == endTime) {
			ret.add(DateExtendUtil.formatDate2SmallDateString(calendar
					.getTime()));
	...
	public static List<String> getBaseListByHour(List<String> listDay) {
		List<String> ret = new ArrayList<String>();
		String[] timePoint = { "0", "1", "2", "3", "4", "5", "6", "7", "8",
	 ...
	 */
	public static Timestamp getTimestampForStringYMD(String date) {
		String dateTime = null;
		if(StringUtils.isNotEmpty(date)){
			if(date.length()==10){
				dateTime = date +" 00:00:00";
			}else if(date.length()==4){
				dateTime = date +"-01-01 00:00:00";
